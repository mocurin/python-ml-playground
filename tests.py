...
class NeuralNetworkTest(unittest.TestCase):
    def test_and(self):
        ...
        self.assertEqual(nn.compute([0, 0]), [0])
        self.assertEqual(nn.compute([0, 1]), [0])
        self.assertEqual(nn.compute([1, 0]), [0])
        self.assertEqual(nn.compute([1, 1]), [1])
    ...
    def test_or(self):
        ...
        self.assertTrue(nn.check_dimensions())
        self.assertEqual(nn.compute([0, 0]), [0])
        self.assertEqual(nn.compute([0, 1]), [1])
        self.assertEqual(nn.compute([1, 0]), [1])
        self.assertEqual(nn.compute([1, 1]), [1])

    def test_xor(self):
        threshold = make_threshold(0.5)
        nn = neural.make_feedforward_neural_network(input_count=2, output_count=1)
        nn.add_layer('linear', matrix=[[1, -1], [-1, 1]], bias=[0, 0])
        ...
        self.assertEqual(nn.compute([0, 1]), [1])
        self.assertEqual(nn.compute([1, 0]), [1])
        self.assertEqual(nn.compute([1, 1]), [0])
